...
func main() {
	log.Println("==================================================================")
	log.Println("      SNMP UDP Packet Capture & Sniffer Service (Docker)          ")
	log.Println("==================================================================")

	portsEnv := os.Getenv("LISTEN_PORTS")
	if portsEnv == "" {
		portsEnv = "21001,21002,21003"
	}
	...
	log.Println("[INFO] Sniffer listening for incoming NMS/OPNsense SNMP packets...")
	<-stopChan
	log.Println("[INFO] Sniffer shutting down gracefully.")
}
...
func listenUDPPort(port int) {
	...
	if err != nil {
		log.Fatalf("[FATAL] Failed to listen on UDP %s: %v", addr, err)
	}
	defer conn.Close()

	log.Printf("[INFO] UDP Listener started on %s\n", addr)
	buf := make([]byte, 65535)
...
func decodeSNMPPacket(data []byte) (*gosnmp.SnmpPacket, error) {
	g := &gosnmp.GoSNMP{
		...
		Retries:   1,
		Logger:    gosnmp.NewLogger(log.New(os.Stderr, "", 0)),
	}
...
func logCapturedPacket(cap CapturedPacket, extraMsg string) {
	...
	fmt.Println("------------------------------------------------------------------")
	fmt.Printf("[CAPTURED SNMP PACKET] Port: %d | From: %s | PDU: %s\n", cap.ListenPort, cap.SrcAddress, cap.PDUType)
